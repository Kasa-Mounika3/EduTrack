...
const listAcademicStructure = asyncHandler(async (req, res) => {
  const [academicYears, departments, sections, subjects] = await Promise.all([
    ...
    Subject.find().populate('course', 'courseName courseCode').populate('department', 'departmentName').populate('section', 'sectionName year').populate('teacher', 'name email').sort({ subjectName: 1 })
  ]);
